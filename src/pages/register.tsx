...
const KreatopAuthForms = () => {
  ...
  const [loading, setLoading] = useState<null | "register" | "github" | "google">(null);
  ...
  const validate = () => {
    if (!formData.name.trim()) {
      toast.error("Oops!", { description: "Please enter your full name" });
      return false;
    }
    if (!formData.email.trim()) {
      toast.error("Oops!", { description: "Please enter your email" });
      return false;
    }
    if (!formData.password) {
      toast.error("Oops!", { description: "Please enter your password" });
      return false;
    }
    if (formData.password.length < 8) {
      toast.error("Oops!", { description: "Password must be at least 8 characters" });
      return false;
    }
    return true;
  };
    ...
    try {
      ...
      if ((res as any)?.error) {
        toast.error("Register failed", {
          description: (res as any).error.message ?? "Could not create account",
        });
        return;
      }

      toast.success("Yeay!", { description: "Your account has been created" });

      // Some setups auto sign-in after sign up, some don't.
      // We'll attempt to go dashboard; if protected, your middleware will redirect.
      window.location.assign("/dashboard");
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : "Something went wrong while registering.";
  ...
  const handleGithub = async () => {
    try {
      ...
      // Social sign up is just social sign in — provider handles account creation.
  ...
  const handleGoogle = async () => {
    toast.error("Not configured", { description: "Add Google provider in Better Auth config first." });
  };
            ...
            <div className="bg-primary p-12 flex flex-col justify-between overflow-hidden">
              <div className="relative z-10">
                ...
                <div className="space-y-4">
                  <h2 className="text-4xl font-bold text-white leading-tight">Start Your Journey</h2>
              ...
              <div className="w-full max-w-md mx-auto">
                <div className="mb-8 flex items-start justify-between">
                  <div className="flex flex-col gap-2">
                    ...
                    <p className="text-[#89869A]">Fill in your details to get started</p>
                  </div>
                  <ModeToggle />
                </div>
                ...
                <div className="space-y-5">
                  ...
                  <Button
                    ...
                    disabled={loading !== null}
                  >
                    <span>{loading === "register" ? "Creating..." : "Create Account"}</span>
                  ...
                  <div className="relative">
                    ...
                    <div className="relative flex justify-center text-sm">
                      <span className="px-4 bg-[#110E24] text-[#89869A]">Or continue with</span>
                    </div>
                  </div>
